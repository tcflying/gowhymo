...
class SelectedTabIndexNotifier extends Notifier<int> {
  ...
  Future<void> select(int index) async {
    state = index;
    // 只在切换到非计划tab时重置主题，但让计划tab保持其自身的主题逻辑
    if (index != 1) {
      await ref
...
class AppUpdateNotifier extends Notifier<UpdateState> {
  ...
  Future<Map<String, dynamic>> checkVersion() async {
      ...
      final response = await dio.get(
        ...
        options: Options(
          ...
          validateStatus: (status) =>
              status! < 600, // Accept status codes less than 500
        ),
      );
      log("version:${response.data}",name: 'ui/home_screen/home_screen_providers.dart');
    ...
    //   'releaseNotes': '这是一个测试版本',
    //   'downloadUrl':
    //       'https://github.com/owhymo/gowhymo/releases/download/v1.0.0/gowhymo.apk',
    // };
  }
  ...
  Future<void> _fetchUpdateState() async {
    try {
      ...
      log(
        '版本检查结果: $updateState',
        name: 'ui/home_screen/home_screen_providers.dart',
      );
    } catch (e, stackTrace) {
      log(
        '版本检查失败: $e',
...
class CalendarWeekNotifier extends Notifier<CalendarWeekState> {
  ...
  CalendarWeekState build() {
    pageController = PageController(initialPage: 4); //向前4周向后4周
  ...
  static List<CalendarDate> _generateWeekDates(DateTime date) {
    ...
    return weekDates.map((day) {
      // 计算农历日期 - 使用chinese_lunar_calendar包转换公历到农历
      ...
      // 处理农历显示文本
      String lunarText = lunar.lunarDay.toString(); // 默认显示农历日

      if (lunar.lunarDay == 1) {
        lunarText = '${chineseMonth[lunar.lunarMonth.number]}月'; // 显示中文数字月份
      } else {
        lunarText = chineseDay[lunar.lunarDay];
      }
      // log(lunarText.toString());
  ...
  Future<void> selected(DateTime date) async {
    ...
    if (isToday) {
      // log('isToday - 设置为dark模式 + 正常主题');
      await ref
          .read(themeModeProvider.notifier)
          .updateThemeMode(ThemeMode.dark);
      await ref.read(isGreyModeProvider.notifier).updateGreyMode(false);
    } else if (isFuture) {
      // log('isFuture - 设置为light模式 + 正常主题');
      await ref
          .read(themeModeProvider.notifier)
          .updateThemeMode(ThemeMode.light);
      await ref.read(isGreyModeProvider.notifier).updateGreyMode(false);
    } else {
      // log('isPast - 设置为dark模式 + 灰色主题');
      await ref
          .read(themeModeProvider.notifier)
          .updateThemeMode(ThemeMode.dark);
